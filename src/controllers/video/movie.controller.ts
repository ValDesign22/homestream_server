...
export class MovieVideoController {
  ...
  public async get(req: Request, res: Response) {
    ...
    if (!movie_item) return res.status(404).json({ message: 'Movie not found' });

    const video_path = movie_item.path;
    if (!video_path || !existsSync(video_path)) return res.status(404).json({ message: 'Video file not found' });
    ...
    if (!existsSync(playlist_path)) {
      try {
        ...
        return res.status(500).json({ message: 'Failed to generate HLS playlist' });
      }
    }
